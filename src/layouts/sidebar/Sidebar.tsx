        ...
        <div className="bg-white dark:bg-black h-full">
          <div className="flex justify-between items-center px-4 py-3">
            <NavLink to="/" className="main-logo flex items-center shrink-0">
              ...
              <span className="text-2xl text-dark ltr:ml-3 rtl:mr-1.5 font-semibold align-middle lg:inline dark:text-white-light">IntelliBook</span>
            </NavLink>
            ...
            <ul className="relative font-semibold space-y-0.5 p-4 py-0">
              {SidebarMenu.map((menu, index) => (
                <li key={index} className="menu nav-item ">
                  ...
                  {/* Sidebar child */}
                  {menu.child && (
                    <button type="button" className={`${currentMenu ? '' : ''} nav-link group w-full`} onClick={() => toggleMenu(`kategori-${index + 1}`)}>
                      <div className="flex items-center">
                        {/* Ganti ini dengan ikon sesuai dengan menu */}
                        <Icon icon={menu.icon} width={22} className="group-hover:!text-primary shrink-0" />
                        <span className="ltr:pl-3 rtl:pr-3 text-dark dark:text-white dark:group-hover:text-white-dark">{menu.title}</span>
